...
def compute_dist(quest_point, past_occup, alpha, beta) : 
    ...
    for domain in dfs.keys() : 
        ...
        dist_df = pd.DataFrame(squareform(pdist(df, 'euclidean')), index=df.index, columns= df.index)
        
        # If domain is MM normalize later
        if not(domain == u'MM') :
            dist_df = (dist_df - np.mean(dist_df.as_matrix())) / np.std(dist_df.as_matrix())    
        
        distance_dfs[domain] = dist_df


    # Sum of values for all pairwise Occupations of all the domains except MM
    sum_domains = sum([distance_dfs[domain] for domain in domains if not domain == u'MM'])
    ...
    final_dist = (alpha * D_mmS + (1-alpha) * D_mmP[past_occup] + beta * sum_domains[past_occup]) / (1 + beta * 9)
